---
company: Freelance Contractor
role: Web Developer/ Marketer
description: As a Freelance Web Developer/Marketer, I collaborated with diverse
  clients, including Royal Wolf Tattoos and Merafly.com, enhancing platforms and
  optimizing user engagement. I contributed to the Paypay Japan PHP SDK on
  GitHub and developed a widgeting platform for Verbinden Communications,
  integrating it into Biocon India's frontpage with server-side rendering in PHP
  and Vue.js. My versatile skills in development and marketing drove impactful
  results across various projects.
startDate: 01.06.2017
endDate: 01.09.2019
orgLogo: /images/uploads/freelance-logo.png
---
▪ Partnered with a diverse clientele, including Royal Wolf Tattoos, Edge Advanced Solutions, Piya International, Saurashtra Agricare,

Critarac Technologies, Verbinden Communication Pvt. Ltd., and Merafly.com.

▪ Collaborated on the development of the Paypay Japan PHP SDK, contributing to its open-source repository on GitHub (GitHub Link: [paypay/paypayopa-sdk-php](https://github.com/paypay/paypayopa-sdk-php)).

▪ Played a pivotal role in enhancing the feature set of the travel ticketing platform, Merafly.com, to optimize user engagement and functionality.

▪ Designed and developed a proprietary widgeting platform for Verbinden Communications, which was seamlessly integrated into the frontpage site for Biocon India. Key features included server-side rendering in PHP with view control using Vue.js.
